...
TWILIO_ACCOUNT_SID = 'AC8748d8d0c268d3c27900977037dded8f'
TWILIO_AUTH_TOKEN = '5f3332d6ccf98b3dc88e4f9223c0061d'
TWILIO_PHONE_NUMBER = '+15303174345'
    ...
    if request.method == 'POST':
        ...
        if conflicting_booking:
            flash_message = (
                ...
                f'{conflicting_booking.start_time.strftime("%H:%M")}-'
                f'{conflicting_booking.end_time.strftime("%H:%M")})'
            )
...
@app.route('/api/slot-bookings/<int:slot_id>')
@login_required
def get_slot_bookings(slot_id):
    slot = Slot.query.get_or_404(slot_id)
    
    # Get the upcoming booking for this slot
    ...
    if not booking:
        return jsonify({'error': 'No active booking found for this slot'})
